...
# Prerequisites: the secondary internal network (VPC) should be set to the 10.100.100.0/24 on DO and the machine should be appointed to the network
# the machine's IP on this network
vpcip=10.100.100.1
...
## VPC CONFIGURATION
# Prerequisites: the secondary internal network (VPC) should be set to the 10.100.100.0/24 on DO and the machine should be appointed to the network
# Changing the default *.*.*.2 IP to *.*.*.1 of the secondary, internal network card

if [[ $(ifconfig | grep eth1) ]]; then
  sed -i 's/10.100.100.*/'$vpcip'/g' /etc/network/interfaces.d/50-cloud-init.cfg
fi
...
adduser --disabled-password --gecos "" ionel
usermod -aG sudo ionel
mkdir -m 700 $sdir
cp ~/.ssh/authorized_keys $sdir
chmod 600 $sdir/authorized_keys
chown -R ionel:ionel $sdir
