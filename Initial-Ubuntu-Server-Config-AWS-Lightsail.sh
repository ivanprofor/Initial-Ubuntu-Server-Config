...
# Allow the user to run root commands without password, only by invoking "sudo su" command before the actual command, by uncommeting the following line
echo '$usr ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers

mkdir -m 700 $sdir
# cp ~/.ssh/authorized_keys $sdir
mv /$udir/authorized_keys $sdir
...
sed -i -re 's/^(Port)([[:space:]]+)22/\1\2'$sshp'/' -e 's/^(LoginGraceTime)([[:space:]]+)120/\1\2'$sshlgt'/' -e 's/^(\#)(Banner)([[:space:]]+)(.*)/\2\3\4/' -e "s/^PermitRootLogin prohibit-password/PermitRootLogin no/" $sshdc;
